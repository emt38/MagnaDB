    ...
    public abstract class TableModel<T> : ViewModel<T> where T : TableModel<T>, new()
    {
        ...
        public static bool CreateTable()
        {
            ...
            foreach (PropertyInfo property in reference.FilterProperties(PresenceBehavior.ExcludeAll, typeof(DMLIgnoreAttribute), typeof(DDLIgnoreAttribute), typeof(ForeignRelationAttribute)))
            {
                ...
                temp.AppendFormat(" {0} {1} {2} {3}, ", columnName?.Name ?? property.Name, dateAttribute != null ? dateAttribute.DateKind == DateTimeSpecification.Date ? "date" : dateAttribute.DateKind == DateTimeSpecification.Time ? "time" : "datetime" : property.PropertyType.ToMySqlTypeNameString(), (property.PropertyType.IsNullable() || property.PropertyType.IsClass) ? "NULL" : "NOT NULL", isIdentity ? " IDENTITY(1,1)" : "");
            }
